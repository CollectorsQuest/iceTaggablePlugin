...
require_once dirname(__FILE__).'/../../lib/IceTaggableBehavior.class.php';

// test variables definition
define('TEST_CLASS', 'Bookstore');
define('TEST_CLASS_2', 'Book');
...
// clean the database
iceModelTagPeer::doDeleteAll();
iceModelTaggingPeer::doDeleteAll();
...
$t->is($object->getTags(), array(), 'a new object has no tag.');
...
$c = new Criteria();
$c->setLimit(3);
$tags = iceModelTagPeer::getPopulars($c);
...
IceTaggableBehavior::preloadTags($tagged_with_tag17);
$nb_tags = 0;
...
$t->ok(IceTaggableToolkit::isTaggable(TEST_CLASS) === true, 'it is possible to tell if a model is taggable from its name.');

$object = _create_object();
$t->ok(IceTaggableToolkit::isTaggable($object) === true, 'it is possible to tell if a model is taggable from one of its instances.');
$t->ok(IceTaggableToolkit::isTaggable('Tristan\'s cat') === false, 'Tristan\'s cat is not taggable, and that is fine.');
...
$t->ok(IceTaggableToolkit::extractTriple('ns:key=value') === array('ns:key=value', 'ns', 'key', 'value'), 'triple extracted successfully.');
$t->ok(IceTaggableToolkit::extractTriple('ns:key') === array('ns:key', null, null, null), 'ns:key is not a triple.');
$t->ok(IceTaggableToolkit::extractTriple('ns') === array('ns', null, null, null), 'ns is not a triple.');
...
$tag = iceModelTagPeer::retrieveOrCreateByTagname('ns:key=value');
$t->ok($tag->getIsTriple(), 'a triple tag created from a string is identified as a triple.');

$tag = iceModelTagPeer::retrieveOrCreateByTagname('tutu');
...
$tags_triple = iceModelTagPeer::getAll(null, array('triple' => true));
$result = array();
...
$tags_triple = iceModelTagPeer::getAll(null, array('triple' => false));
$result = array();
...
$tags_triple = iceModelTagPeer::getAll(null, array('triple' => true, 'namespace' => 'ns'));
$result = array();
...
$tags_triple = iceModelTagPeer::getAll(null, array('triple' => true, 'key' => 'key'));
$result = array();
...
$tags_triple = iceModelTagPeer::getAll(null, array('triple' => true, 'value' => 'tutu'));
$result = array();
...
$objects_triple = iceModelTagPeer::getTaggedWith(array(), array('namespace' => 'ns', 'model' => TEST_CLASS));
...
sfConfig::set('app_IceTaggableBehaviorPlugin_triple_distinct', true);
$t->diag('querying triple tagging');
...
$t->todo('it is possible to set up the plugin so that namespace:key is a unique key.');
...
$t->todo('it is possible to apply triple tags to various objects when the plugin is set up so that namespace:key is a unique key.');
